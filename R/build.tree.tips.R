   ...
   tree.alive <- FALSE
    ...
    while (any(c(length(open.edges) != extant.tips, mass.ext.occ))) {
        if (tree.alive == FALSE) {
            ...
            edge.lengths <- c(0, 0)
            n.tips <- total.time <- 0
            root.state <- mvrnorm(n = 1, mu = rep(0, n.characters), 
                Sigma = bm.rates)
            withinBounds <- FALSE
            if (all(c(root.state <= upperBound, root.state >= 
                lowerBound))) 
                withinBounds <- TRUE
        ...
        edge.lengths[open.edges] <- edge.lengths[open.edges] + 
            waiting.time
            ...
            function(branch.vals) {
                ...
                while (withinBounds == FALSE) {
                  branch.vals.temp <- last.time + mvrnorm(n = 1, 
                    mu = rep(0, n.characters), Sigma = waiting.time * 
                      bm.rates)
                  if (all(c(branch.vals.temp <= upperBound, branch.vals.temp >= 
                    lowerBound))) 
                    withinBounds <- TRUE
                }
                rbind(branch.vals, branch.vals.temp)
            })
        time.list[open.edges] <- lapply(time.list[open.edges], 
            function(x) c(x, total.time))
        n.tips <- length(which(start.edge[, 2] > 0))
        if (spp.or.ext) {
            free.edges.values <- sapply(branch.value.list[open.edges], 
                function(x) x[dim(x)[1]])
            if (both.sides) {
                diff.mean.val <- abs(free.edges.values - mean(free.edges.values))
                spp.chance.rank <- rank(diff.mean.val)
                if(spp.size.prob < 0) spp.chance.rank <- (max(spp.chance.rank) + 1) - spp.chance.rank		
                spp.chance <- spp.chance.rank ^ abs(spp.size.prob)
            	} else {
            	spp.value.order <- rank(free.edges.values)
            	if(spp.size.prob < 0) spp.value.order <- (max(spp.value.order) + 1) - spp.value.order
            ...
            start.edge <- rbind(start.edge, matrix(c(new.label, 
                new.label, NA, NA), 2, 2))
            edge.lengths <- c(edge.lengths, 0, 0)
            node.temp <- tail(branch.value.list[[open.edges[spp.edge]]], 
                1)
            ...
            if (both.sides) {
                ...
                if(ext.size.prob > 0)  ext.chance <- (max(ext.chance) + 1) - ext.chance
                ext.chance <- ext.chance ^ abs(ext.size.prob)
            	} else {
            	ext.risk.order <- rank(free.edges.values)
            	if(ext.size.prob < 0) ext.risk.order <- (max(ext.risk.order) + 1) - ext.risk.order
        ...
        if (length(open.edges) == (mass.ext.tips) && mass.ext.occ) {
            ...
            time.list[open.edges] <- lapply(time.list[open.edges], 
                function(x) c(x, total.time))
            free.edges.values <- t(sapply(branch.value.list[open.edges], 
                function(x) x[dim(x)[1], ]))
            if (n.characters == 1) 
            ...
            if (length(ext.casualty) > extinct.check) {
                ext.casualty <- sample(ext.casualty, floor(mass.ext.severity * 
                  length(open.edges)))
            }
            length.not.strict <- length(open.edges[-ext.casualty])
            vector.of.probs <- c(rep(1, length.strict), rep(chance.of.random, 
                length.not.strict))
            ...
            if (chance.of.random <= 1e-08) {
                ...
                ext.casualty <- sample(c(ext.casualty, non.casualty), 
                  length.strict, prob = vector.of.probs)
            }
            else {
                rand.ext <- 1:length(vector.of.probs)
                non.casualty <- rand.ext[-ext.casualty]
    ...
    time.list[open.edges] <- lapply(time.list[open.edges], function(x) c(x, 
        total.time))
    ...
    tips <- c(which(start.edge[, 2] == 0), which(is.na(start.edge[, 
        2])))
    n.tips <- length(tips)
    edge.dim <- dim(start.edge)[1]
    if (((n.tips * 2 - 2) != edge.dim)) 
        warning("this is not a tree")
    continuous.seq <- match(seq(1:max(start.edge, na.rm = T)), 
        c(start.edge))
    cont.na <- which(is.na(c(continuous.seq)))
    if (length(cont.na) > 0) {
        edge.one <- which(start.edge[, 1] > cont.na)
        start.edge[edge.one, 1] <- start.edge[edge.one, 1] - 
            1
        edge.two <- which(start.edge[, 2] > cont.na)
        start.edge[edge.two, 2] <- start.edge[edge.two, 2] - 
            1
    }
    ...
    orig.outer <- start.edge[which(start.edge[, 2] > n.tips), 
        2]
    start.edge[which(start.edge[, 2] > n.tips), 2] <- sort(orig.outer)
    start.edge.orig.2 <- start.edge[, 1]
    for (p in 1:length(sort(orig.outer))) start.edge[, 1][which(orig.outer[p] == 
        start.edge.orig.2)] <- sort(orig.outer)[p]
    ...
    ltt.measure <- cbind(node.times = c(0, ltt.times), lineages = c(0, 
        as.numeric(tab.time.ltt)))
    ...
    trait.ltt <- lapply(signif(ltt.measure[, 1]), function(t.l) {
        ...
        for (q in 1:n.characters) {
            unlist.traits <- unlist(sapply(branch.value.list[re.order], 
                function(x) x[, q]))

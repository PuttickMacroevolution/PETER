...
#' @description Simulates a birth-death phylogeny with simultaneous evolution under Brownian motion for one or more traits. The simulations continue until a certain time has elapsed since the start of the process. Additionaly, a mass extinction can occur with a user-set intensity of extinction after a certain time within the process. Traits can be correlated or independent, can be linked to increased rates of speciation and/or extinction at background times, and can drive selectivity at mass extinction event. 
#' @param lambda Speciation rate
#' @param mu Extinction rate. If mu=0, then the simulation follows a Yule pure birth process
#' @param time.limit the point at which to end the simulation measured from the origin of the process (time 0). Must be > 0.
#' @param time.limit.mass.ext the point at which mass extinction occurs, not applicable if mass.ext.occ=FALSE
...
#' @param trait.axes If univariate.selection=FALSE, the trait axes on which selection will occur. The default (NULL) acts on all trait axes
...
build.tree.time <- function(lambda=1, mu=0, time.limit=4, time.limit.mass.ext=2, mass.ext.quantile=0.5, n.characters=1, sigma.rates=1, spp.size.prob=0, ext.size.prob=0, larger.than=TRUE, mass.ext.occ=TRUE, mass.ext.severity=0.5, trait.axes = NULL, chance.of.random = 0.1,  upperBound=Inf, lowerBound=-Inf, return.trait.matrix=TRUE, return.branch.value.list=FALSE, return.time.list=FALSE)  {
	...
	while (any(c(total.time < time.limit, mass.ext.occ))) {
		...
		if (total.time >= time.limit.mass.ext && mass.ext.occ) {
			# stop()
			...
			if(! is.null(trait.axes)) {
				...
				if (dim(trait.sel)[1] == 1) trait.sel <- t(trait.sel)
			}
			if (larger.than) {

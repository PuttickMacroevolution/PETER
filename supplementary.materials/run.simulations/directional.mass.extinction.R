...
# for(mu.val in c(0, 0.4, 0.8)) {
	# for(y in tip.number) {
		# for(u in severity) {
			# for(i in random) {
				# disp.out <- mclapply(1:100, mc.cores=n.cores, function(in.lo) {
					# est.out <- suppressWarnings(build.tree.tips(lambda=1, mu=mu.val, extant.tips=y, mass.ext.occ=TRUE, sigma.rates=1, mass.ext.tips=y, mass.ext.quantile=1-u, mass.ext.severity=u, chance.of.random=i, trait.axes=1))
						# }
					# )
			...
			# for(u in severity) {
				# for(i in random) {
					# disp.out.int <- mclapply(1:100, mc.cores=8, function(in.lo) {
						# est.out <- build.tree.tips(lambda=1, mu=mu.val, extant.tips=y, mass.ext.occ=TRUE, sigma.rates=bm.val, mass.ext.tips=y, mass.ext.quantile=1-u, mass.ext.severity=u, chance.of.random=i, n.characters=2, trait.axes=1)
						# est.out
							# }
						# )
	...
	# for(mu.val in c(0, 0.4, 0.8)) {
		# for(y in tip.number) {
			# for(u in severity) {
				# for(i in random) {
					# disp.out.int <- mclapply(1:100, mc.cores=8, function(in.lo) {
						# est.out <- build.tree.tips(lambda=1, mu=mu.val, extant.tips=y, mass.ext.occ=TRUE, sigma.rates=bm.val, mass.ext.tips=y, mass.ext.quantile=1-u, mass.ext.severity=u, chance.of.random=i, n.characters=5, trait.axes=1)
						# est.out
							# }
						# )

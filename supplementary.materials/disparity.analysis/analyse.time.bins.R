...
## estimate disparity for timebins and by sub-sampling data
	all.outs <- mclapply(1:length(sigma.files), mc.cores = n.cores,
	  FUN = function(count) {
		...
		for (bin.number in c(2, 4, 5, 8)) {
						...
						x.out <- sapply(x2, function(x) {
							...
							if (nrow(x.in) > 2) {
								euclid.out <- unlist(dispRity(x.in,
								  metric = c(median, pairwise.dist),
									method = "euclidean")$disparity)
								if (n.col == 1)
								  x.in <- cbind(as.matrix(x), 0)
								sov.out <- unlist(dispRity(x.in,
								  metric = c(sum, variances))$disparity)
								sor.out <- unlist(dispRity(x.in,
								  metric = c(sum, ranges))$disparity)
								if(n.col == 1)
								  x.in <- cbind(as.matrix(x), in.file[[xx]]$root.state)
								centroid.out <- unlist(dispRity(x.in,
								  metric = c(centroids, median),
									centroid = root.state)$disparity)
			...
			colnames(difference.in.metric) <-
			  c("euclidean", "sov", "sor", "centroid", "n_bin1", "n_bin2")
			list.full[[counter]] <- difference.in.metric
			counter <- counter + 1
			}
		}
		names(list.full) <- c(paste("four bins", c(1, 0.5, 0.1, 0.01)),
		  paste("eight bins", c(1, 0.5, 0.1, 0.01)),
			paste("ten bins", c(1, 0.5, 0.1, 0.01)),
			paste("sixteen bins", c(1, 0.5, 0.1, 0.01)))
		list.full
	})
	names(all.outs) <- sigma.files
	saveRDS(all.outs,
	  "../disparity.analysis/outputs/tips_50.mu_0.4.severity_0.5.random_0.01_bin")	
